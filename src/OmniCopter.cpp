...
void OmniCopter::getRawInput(struct Input_Raw* input,int* RC1)
{
  ...
  //记录下最原始的数据
	...
	this->rawInput.Roll[0] = input->Roll[0];
  this->rawInput.Roll[1] = input->Roll[1];
  this->rawInput.Roll[2] = input->Roll[2];
...
void getAngle(Angle angle) {
	angle.pitch = asin(-2 * QURT1 * QURT3 + 2 * QURT0* QURT2);
	angle.roll = atan2(2 * QURT2 * QURT3 + 2 * QURT0 * QURT1, -2 * QURT1 * QURT1 - 2 * QURT2* QURT2 + 1);
	angle.yaw = atan2(2*(QURT1*QURT2 + QURT0*QURT3),QURT0*QURT0+QURT1*QURT1-QURT2*QURT2-QURT3*QURT3);
}
...
void OmniCopter::collectAngleError() {
  ...
  //累计角度误差
  this->angleErrorCollection.roll  += this->rawInput.Roll[0] - this->nowAngle.roll ;
  this->angleErrorCollection.pitch += this->rawinput.Roll[1] - this->nowAngle.pitch ;
  this->angleErrorCollection.yaw   += this->rawinput.Roll[2] - this->nowAngle.yaw ;
